...
```{r missing_homeplanet, fig.cap = "Missing values for HomePlanet"}
train2 %>%
  group_by(PassengerGroup) %>%
...
```{r missing_cabin, warning = FALSE, fig.cap = "Missing values for Cabin"}
train2 %>%
...
```{r missing_deck, fig.cap = "Missing values for Deck"}
train2 %>%
  group_by(Deck) %>%
  filter(any(is.na(HomePlanet))) %>%
  ungroup() %>%
...
```{r missing_age, warning = FALSE, fig.cap = "Missing values for ameneties"}
train2 %>%
...
```{r missranger_imputation, warning = FALSE}
...
```{r knn_imputation, warning = FALSE}
...
```{r imputation_evaluation_num, fig.cap = "Comparisson of standard deviation for numerical values before and after imputation"}
...
```{r imputation_evaluation_num2, fig.cap = "Comparisson of mean for numerical values before and after imputation"}
mean_skim %>%
  filter(Metric != "No imp") %>%
...
```{r imputation_evaluation_cat, echo = FALSE, fig.cap = "Imputed distribution for categorical variables with all three methods against no imputation"}
...
```{r manual_imp_correction, fig.cap = "Missing values overview after imputation"}
