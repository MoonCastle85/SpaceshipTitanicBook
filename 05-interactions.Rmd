...
```{r interactions-cat, warning = FALSE, cache=TRUE, message=FALSE}
...
# 
# plotsToSVG <- walk2(.x = cat_pair_int_plots, .y = seq_along(cat_pair_int_plots), .f = save_plot)
# plotsToSVG <- map(1:length(cat_pair_int_plots), .f = \(i) paste0("PairInt", i, ".png"))
# save(plotsToSVG, file = "Plots cat.RData")
...
```{r interactions-all, warning = FALSE, message=FALSE, cache=TRUE}
...
# 
# plotsToSVG2 <- walk2(.x = num_pair_int_plots, .y = seq_along(num_pair_int_plots), .f = save_plot2)
# plotsToSVG2 <- map(1:length(num_pair_int_plots), .f = \(i) paste0("PairIntNum", i, ".png"))
# save(plotsToSVG2, file = "Plots num.RData")
load("Plots num.RData")

slickR::slickR(plotsToSVG2[1:45], height = "480px", width = "672px") +
  slickR::settings(slidesToShow = 1, dots = TRUE)
```

```{r interactions-all2, warning = FALSE, message=FALSE, cache=TRUE}
slickR::slickR(plotsToSVG2[46:90], height = "480px", width = "672px") +
  slickR::settings(slidesToShow = 1, dots = TRUE)
```
...
```{r pen-reg-int-top, cache=TRUE, warning = FALSE, message = FALSE, fig.cap = "Top variables from the Lasso/Ridge regression with interaction effects."}
...
It's surprising to see that the strongest effect was between Deck & Side, which we visually inspected in Figure \@ref(fig:int-deck-side) where it looked as if the effect of the interaction wasn't very large. It also didn't show up in our pairwise exploration which perhaps indicates that this interaction is overfitted. 
...
The tree model heavily favours the amenity variables as well as a few other variables that our penalized model didn't consider important enough. Based on the ranking in Figure \@ref(fig:tree-based-var-imp2), there is a clear cutoff after the LastNameCount feature. 
