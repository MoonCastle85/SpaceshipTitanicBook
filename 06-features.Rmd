...
load("Extra/Recursive feature elimination with interactions removed correlated.RData")
```
...
# })
# 
# save(sim_anneal_50_pct, file = "Extra/Simulated annealing with 50% inital.RData")
# 
# stopCluster(my_cluster)
# unregister()
...
p_tests <- data.frame(test = rep(c("Wilcoxon paired", "t.test paired"), 3),
                      colour_grp = rep(c("RFE Pen", "SA", "GA"), 2),
                      value = c(p_wilcoc_rfe_pen, p_wilcoc_sa, p_wilcoc_ga, 
                                p_ttest_rfe_pen, p_ttest_sa, p_ttest_ga))
...
ggplot(p_tests, aes(x = test, y = value, colour = colour_grp)) +
  ...
  scale_colour_manual(values = c("RFE Pen" = "green", "SA" = "orange", "GA" = "red", "size" = "none")) +
...
save(best_interactions, file = "Extra/Best interactions.RData")
